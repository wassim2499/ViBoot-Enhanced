...
let is_venue = (i) => {
	...
	if (empty_venue_count > 2) return false;
	else return true;
};
	...
	chrome.storage.sync.get(['token'], (token) => {
		if (token.token != null) {
			for (let i = 0; i < row.length; i++) {
				if (
					row[i].children[0].className === 'panelHead-secondary' &&
					is_venue(i)
				) {
			...
			let get_details = (i) => {
				...
				i++;
				let row_count =
					document.getElementsByTagName('tbody')[0].rows.length;
				...
				while (
					temp_row[i].children[0].className !== 'panelHead-secondary'
				) {
			...
			let get_date_time = (date, time) => {
				...
				if (start_date_time.slice(-2).toLowerCase() == 'pm') {
					let clk_24 = (
						parseInt(start_date_time.slice(0, 2)) + 12
					).toString();
					if (clk_24 >= 24) clk_24 = '12';
					start_date_time =
						date +
						'T' +
						clk_24 +
						start_date_time.slice(2, 5) +
						':00.000+05:30';
				} else {
					start_date_time =
						date +
						'T' +
						start_date_time.slice(0, 5) +
						':00.000+05:30';
				}
				if (end_date_time.slice(-2).toLowerCase() == 'pm') {
					let clk_24 = (
						parseInt(end_date_time.slice(0, 2)) + 12
					).toString();
					if (clk_24 >= 24) clk_24 = '12';
					end_date_time =
						date +
						'T' +
						clk_24 +
						end_date_time.slice(2, 5) +
						':00.000+05:30';
				} else {
					end_date_time =
						date +
						'T' +
						end_date_time.slice(0, 5) +
						':00.000+05:30';
				}
			...
			let sync_dates = (details, j) => {
				chrome.storage.sync.get(['token'], (token) => {
					...
					let [start_time, end_time] = get_date_time(
						dte,
						details.exam_time[j],
					);
					fetch(
						'https://www.googleapis.com/calendar/v3/calendars/primary/events?sendUpdates=all&sendNotifications=true&alt=json&key=AIzaSyCPBz-DTZdoTLQ_ZiqsVUO520XItcomTn0',
						{
							...
							body: JSON.stringify({
								...
								eventType: 'default',
								description:
									details.course_code[j] +
									'-' +
									details.venue[j] +
									'-' +
									details.seat_location[j] +
									'-' +
									details.exam_time[j],
								summary:
									details.venue[j] +
			...
			let call_sync = async (i) => {
				...
				let wait_txt =
					document.getElementsByClassName('form-group')[2].children[0]
						.children[0];
				wait_txt.style.textAlign = 'center';
				wait_txt.innerText =
					'Please Wait while the dates get synced!!!';
				for (let j = 0; j < details.course_code.length; j++) {
					...
					if (j == details.course_code.length - 1) {
						wait_txt.innerText = '';
						alert(
							'Venues are successfully Synced to your calendar',
						);
					}
					await sleep(500);
				}
